      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">Broadcast announcements and targeted alerts for {profile?.displayName || user.email}.</p>
          </div>
        ...
        {loading ? (
          <Card className="border-slate-200 bg-white shadow-sm">
            <CardContent className="p-6 text-sm text-slate-500">Loading notifications...</CardContent>
          </Card>
        ) : notifications.length > 0 ? (
          <div className="space-y-4">
            {notifications.map((notification) => (
              <Card key={notification.id} className="border-slate-200 bg-white shadow-sm">
